<template xmlns:v-slot="http://www.w3.org/1999/XSL/Transform">
  <div id="pages__user">
    ...
    <v-btn color="primary" class="float-right" :to="localePath({ name: 'users-create' })">
     {{$t('pageTitle.user.add')}}
    </v-btn>
    <v-data-table
      :headers="headers"
      :items="records"
    ...
    >
      ...
      <template v-slot:top>
        <data-table-top :title="$t('menuTitle.user')" />
      </template>
        ...
        <v-tooltip bottom>
          <template v-slot:activator="{ on }">
            <v-btn
              ...
              small
              v-on="on"
              :to="localePath({ name: 'users-id', params: { id: item.id } })"
            >
...
import FormSearch from "@/components/pages/user/FormSearch";
