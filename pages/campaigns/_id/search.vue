<template>
  <vx-card ref="filterCard" title="Filters" class="user-list-filters mb-8" actionButtons @refresh="resetItem"
    ...
    <div class="vx-row">
      ...
      <div class="vx-col md:w-1/4 sm:w-1/2 w-full">
        <label class="text-sm opacity-75">Keyword</label>
        <vs-input class="w-full mb-4 md:mb-0" v-model="dataKeyword"/>
      </div>
      <div class="vx-col md:w-1/4 sm:w-1/2 w-full">
  ...
  export default {
    ...
    methods: {
      ...
      async resetItem() {
        ...
        await this.$store.dispatch(this.moduleName + "/fetchItems", {campaign_id: this.$route.params.id});
      },
    },
  };
</script>
